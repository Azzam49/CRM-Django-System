...
def registerPage(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #this will create a customer group for the new created user
            group = Group.objects.get(name='customer')
            user.groups.add(group)
            #this will create customer object for the new created user
            Customer.objects.create(
               user=user,
               name = user.username,#assign name to Customer for __Str__ usement
            )

            messages.success(request, f'Accout for {username} is created!')
...
def userPage(request):
    #we are able to get customer information from the request user 
    #by the OneToOne Relation with User and Customer models
